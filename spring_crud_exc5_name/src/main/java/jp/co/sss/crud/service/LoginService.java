...
public class LoginService {
	...
	public LoginResultBean execute(LoginForm loginForm) {
		...
		// DB検索を行いたい場合、①Form（入力値をどこからもらうか）→ ②Repository（DBに何ができるか見る）→③ Entity（今回ならEmployee）
		// テーブルの代わりとなるentityパッケージの中に入っているEmployeeに代入するための記述。
		// フィールドで記述した、repository(EmployeeRepository)の中にある、今回調べたいのはIDとパスワードなのでそれが書かれている
		// findByEmpIdAndEmpPassメソッドを使う。
		// 引数には、executeの引数にした（Loginのする為の）Formクラスの中のIdとパスを取得するためのメソッド。
